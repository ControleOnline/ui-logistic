...
const TABS = [
  ...
  {key: 'cte', label: 'CTE', storeName: 'invoice_tasks_processing', requestParams: {invoiceModel: 57}, actions: 'cte'},
];
...
export default function CtePendingInvoicesPage() {
  ...
  useEffect(() => {
    ...
    tableStore?.actions?.getItems?.({page: 1, itemsPerPage: 50, ...(current.requestParams || {})});
  }, [current.storeName, current.key]);

  const rowActionsComponent = useMemo(() => {
    if (current.actions === 'cte' || current.key === 'cte' || current.key === 'emitted') {
      return CteCteActions;
    }
    return undefined;
  }, [current.actions, current.key]);
    ...
    <SafeAreaView style={styles.screen} edges={['bottom']}>
      <View style={styles.header}>
        ...
        <View style={styles.tabs}>
          {TABS.filter(item => item.key !== 'cte' || true).map(item => (
