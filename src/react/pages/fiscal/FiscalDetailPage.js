...
import {
  ...
  resolveFiscalNumber,
  resolveFiscalProtocol,
  resolveFiscalSeries,
  resolveFiscalStatus,
  resolveInvoiceTaxId,
} from '@controleonline/ui-logistic/src/shared/fiscalDocuments';

const Field = ({label, value, documentType}) => (
  <View style={styles.field}>
    <Text style={styles.label}>{label}</Text>
    <Text testID={`${documentType}-detail-${label}`} style={styles.value}>{String(value || '--')}</Text>
  </View>
);

export default function FiscalDetailPage({documentType}) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!invoiceTaxId) {
        setError(`InvoiceTax da ${config?.title || 'nota'} não informado.`);
        setLoading(false);
        return;
      }
      setLoading(true);
  ...
  }, [invoiceTaxId, config?.title]);

  const valid = isValidFiscalDocument(item);
  const filename = buildFiscalPdfFilename(item, documentType);
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.title}>Detalhe da {config?.title || 'nota'}</Text>
        {loading ? <Text style={styles.hint}>Carregando InvoiceTax #{invoiceTaxId}…</Text> : null}
        {error ? <Text testID={`${documentType}-detail-error`} style={styles.error}>{error}</Text> : null}
        {item ? (
          <>
            <Field documentType={documentType} label="id" value={resolveInvoiceTaxId(item)} />
            <Field documentType={documentType} label="serie" value={resolveFiscalSeries(item)} />
            <Field documentType={documentType} label="numero" value={resolveFiscalNumber(item)} />
            <Field documentType={documentType} label="status" value={resolveFiscalStatus(item)} />
            <Field documentType={documentType} label="protocolo" value={resolveFiscalProtocol(item)} />
            <Pressable
              testID={`${documentType}-detail-pdf`}
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              ...
              <Pressable testID={`${documentType}-detail-pdf-close`} onPress={() => setPreview({visible: false, title: '', url: '', filename: '', loading: false, error: ''})}>
                <Text style={styles.modalClose}>Fechar</Text>
              </Pressable>
            </View>
            {preview.loading ? <Text style={styles.hint}>Carregando {preview.filename}…</Text> : null}
            {preview.error ? <Text testID={`${documentType}-detail-pdf-error`} style={styles.error}>{preview.error}</Text> : null}
            {preview.url
